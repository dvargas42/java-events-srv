...
public class AISearchService {
    ...
    public AISearchJsonOut generateJsonResponse(String prompt) throws Exception{
        ChatCompletionRequest chatRequestQuery = createChatRequest(prompt, AISystemEnum.SYSTEM_QUERY.value());
    ...
    public AISearchMarkDownOut generateMarkDownResponse(String prompt) throws Exception {
        ...
        ChatCompletionRequest chatRequestMarkDown = createChatRequest(queryResult.toString(), AISystemEnum.SYSTEM_MARKDOWN.value());
        String markdownResult = sendChatRequest(chatRequestMarkDown);

        int inputCountTokens = countTokens(
        ...
        BigDecimal costEstimate = costCalculator(inputCountTokens, outputCountTokens, prompt, query);
        return new AISearchMarkDownOut(markdownResult, inputCountTokens, outputCountTokens, costEstimate);
    }
    ...
    private String sendChatRequest(ChatCompletionRequest chatCompletionRequest) throws Exception {
        ...
        throw new AISearchSendChatException("Failed to get a response from the AI service after multiple attempts.");
    }
    ...
    private BigDecimal costCalculator(Integer inputCountTokens, 
            Integer outputCountTokens, String prompt, String system) {
        ...
        BigDecimal inputPricing = model.equals("gpt-3.5-turbo") ? 
                BigDecimal.valueOf(3.00) : BigDecimal.valueOf(0.60);
        BigDecimal outputPricing = model.equals("gpt-3.5-turbo") ? 
                BigDecimal.valueOf(6.00) : BigDecimal.valueOf(2.40);
